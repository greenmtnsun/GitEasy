...
Describe 'Reset-Login (unit contract)' {
    ...
    It 'is exported by the GitEasy module' {
        @(Get-Command -Module GitEasy -Name 'Reset-Login').Count | Should -Be 1
    }

    It 'has CmdletBinding' {
        $cmd = Get-Command 'Reset-Login'
    ...
    It 'has a non-empty .SYNOPSIS' {
        $help = Get-Help 'Reset-Login' -ErrorAction SilentlyContinue
        $help.Synopsis | Should -Not -BeNullOrEmpty
    }

    It 'has at least one .EXAMPLE in CBH' {
        $help = Get-Help 'Reset-Login' -Full -ErrorAction SilentlyContinue
