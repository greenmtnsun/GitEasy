...
$RelativePath  = 'Private\Remove-GEOldLog.ps1'
$SourcePath    = Join-Path $ProjectRoot $RelativePath
}

Describe 'Remove-GEOldLog (private contract)' {
    ...
    It 'declares a function whose name matches the file name' {
        $script:Fn.Name | Should -Be 'Remove-GEOldLog'
    }
    ...
    It 'declares the -Directory parameter' {
        $names = @()
        ...
        $names -contains 'Directory' | Should -Be $true
    }

    It '-Directory is mandatory' {
        $param = $script:Fn.Body.ParamBlock.Parameters | Where-Object { $_.Name.VariablePath.UserPath -eq 'Directory' } | Select-Object -First 1
        $isMandatory = $false
