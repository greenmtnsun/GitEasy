...
$RelativePath  = 'Private\Add-GELogStep.ps1'
$SourcePath    = Join-Path $ProjectRoot $RelativePath

Describe 'Add-GELogStep (private contract)' {
    ...
    It 'declares a function whose name matches the file name' {
        $script:Fn.Name | Should Be 'Add-GELogStep'
    }
    ...
    It 'declares the -Step parameter' {
        $names = @()
        if ($script:Fn.Body.ParamBlock -and $script:Fn.Body.ParamBlock.Parameters) {
            $names = @($script:Fn.Body.ParamBlock.Parameters | ForEach-Object { $_.Name.VariablePath.UserPath })
        }
        $names -contains 'Step' | Should Be $true
    }

    It '-Step is mandatory' {
        $param = $script:Fn.Body.ParamBlock.Parameters | Where-Object { $_.Name.VariablePath.UserPath -eq 'Step' } | Select-Object -First 1
        $isMandatory = $false
        foreach ($attr in $param.Attributes) {
            if ($attr.TypeName.Name -eq 'Parameter') {
                foreach ($na in $attr.NamedArguments) {
    ...
    It 'declares the -ExitCode parameter' {
        $names = @()
        if ($script:Fn.Body.ParamBlock -and $script:Fn.Body.ParamBlock.Parameters) {
            $names = @($script:Fn.Body.ParamBlock.Parameters | ForEach-Object { $_.Name.VariablePath.UserPath })
        }
        $names -contains 'ExitCode' | Should Be $true
    }

    It 'declares the -Output parameter' {
        $names = @()
        if ($script:Fn.Body.ParamBlock -and $script:Fn.Body.ParamBlock.Parameters) {
            $names = @($script:Fn.Body.ParamBlock.Parameters | ForEach-Object { $_.Name.VariablePath.UserPath })
        }
        $names -contains 'Output' | Should Be $true
    }
    ...
    It 'declares the -Note parameter' {
        ...
        $names -contains 'Note' | Should Be $true
    }
