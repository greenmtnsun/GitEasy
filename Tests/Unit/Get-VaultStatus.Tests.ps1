...
Describe 'Get-VaultStatus (unit contract)' {
    ...
    It 'is exported by the GitEasy module' {
        @(Get-Command -Module GitEasy -Name 'Get-VaultStatus').Count | Should Be 1
    }

    It 'has CmdletBinding' {
        $cmd = Get-Command 'Get-VaultStatus'
        $cmd.CmdletBinding | Should Be $true
    }

    It 'has a non-empty .SYNOPSIS' {
        $help = Get-Help 'Get-VaultStatus' -ErrorAction SilentlyContinue
        $help.Synopsis | Should Not BeNullOrEmpty
    }

    It 'has at least one .EXAMPLE in CBH' {
        $help = Get-Help 'Get-VaultStatus' -Full -ErrorAction SilentlyContinue
