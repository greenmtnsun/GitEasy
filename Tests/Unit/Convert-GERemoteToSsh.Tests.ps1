...
$RelativePath  = 'Private\Convert-GERemoteToSsh.ps1'
$SourcePath    = Join-Path $ProjectRoot $RelativePath

Describe 'Convert-GERemoteToSsh (private contract)' {
    ...
    It 'declares a function whose name matches the file name' {
        $script:Fn.Name | Should Be 'Convert-GERemoteToSsh'
    }
    ...
    It 'declares the -RemoteUrl parameter' {
        ...
        $names -contains 'RemoteUrl' | Should Be $true
    }

    It '-RemoteUrl is mandatory' {
        $param = $script:Fn.Body.ParamBlock.Parameters | Where-Object { $_.Name.VariablePath.UserPath -eq 'RemoteUrl' } | Select-Object -First 1
        $isMandatory = $false
