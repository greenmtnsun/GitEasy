...
# --- generate per-doc PDFs into the staged docs folder ---
# PDFs are derived artifacts: never committed to git, always regenerated
# from the staged Markdown so the .pdf and .md ship in lockstep. The
# generator requires PS 7+ (ConvertFrom-Markdown), so we shell out to
# pwsh.exe explicitly. If pwsh.exe is not available we warn and ship
# without PDFs - they are best-effort, not a publish blocker.
$pwshExe = Get-Command pwsh.exe -ErrorAction SilentlyContinue
$builder = Join-Path $repo 'tools\Build-DocsPdf.ps1'
if ($pwshExe -and (Test-Path $builder)) {
    Write-Host "==> Building per-doc PDFs (pwsh -> Edge headless)..." -ForegroundColor Cyan
    & $pwshExe.Source -NoProfile -File $builder `
        -ProjectRoot $repo `
        -OutputDir $stageDocs 2>&1 |
        Where-Object { $_ -notmatch '^\s*$' } |
        ForEach-Object { Write-Host "    $_" -ForegroundColor DarkGray }
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "    PDF build returned exit $LASTEXITCODE - shipping without PDFs."
    }
} else {
    Write-Warning "Skipping PDF build: pwsh.exe not found or tools\Build-DocsPdf.ps1 missing."
}
